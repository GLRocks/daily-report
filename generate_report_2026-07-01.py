#!/usr/bin/env python3
"""Generate 2026-07-01 Agentic Market Daily HTML report."""

import csv
from datetime import datetime

# Stock data for 2026-07-01
stocks = [
    # (ticker, name, price, pct_change, rec, cat, metrics, reason)
    ("NVDA", "NVIDIA Corporation", 200.36, 4.06, "BUY", "芯片", "市值$5.4T | P/S 35x | 毛利率75%", "推理需求结构从训练向推理转移，NVDA软件生态锁定最深"),
    ("AMD", "Advanced Micro Devices", 582.03, 11.59, "BUY", "芯片", "YTD+65% | MI400出货Q3 | 服务器CPU TAM$120B", "MI400系列在推理性价比上挑战NVDA，Lisa Su指引TAM年增35%"),
    ("QCOM", "Qualcomm Inc.", 184.79, -2.43, "SPEC BUY", "芯片", "Q3指引$9.2-10B(低于$10.2B共识) | Android收入减速", "短期业绩miss但AI PC/Auto长期布局 intact，等待回调后加仓窗口"),
    ("TSM", "Taiwan Semiconductor", 477.98, 10.55, "BUY", "芯片", "2nm量产2025H2 | 美国凤凰厂高量投产 | 70%先进制程市占", "先进制程绝对垄断地位，地缘风险已price in部分，产能持续扩张"),
    ("AVGO", "Broadcom Inc.", 377.80, 3.50, "HOLD", "芯片", "定制AI芯片收入$12B/年 | VMware整合完成 | 毛利率80%+", "Google/Meta定制芯片核心供应商，AI ASIC趋势最大受益者"),
    ("MU", "Micron Technology", 1158.05, 2.27, "HOLD", "芯片", "HBM3E量产 | DDR5供需紧 | 内存周期复苏确认", "HBM3E供不应求，AI服务器内存密度提升驱动长期需求"),
    ("AMAT", "Applied Materials", 724.46, 15.57, "HOLD", "芯片", "BIS罚款$300M已消化 | 中国设备收入占比18% | 刻蚀龙头", "先进封装设备需求爆发，HBM/3D封装核心设备供应商"),
    ("LRCX", "Lam Research", 433.91, 14.46, "HOLD", "芯片", "刻蚀/沉积双龙头 | 存储设备周期复苏 | 毛利率47%", "存储资本开支回暖带动设备需求，先进工艺刻蚀复杂度提升"),
    ("ASML", "ASML Holding", 1989.06, 10.83, "HOLD", "芯片", "EUV垄断 | High-NA EUV 2028量产 | 订单积压$40B+", "光刻绝对垄断，High-NA技术护城河加深，长期订单可见性最强"),
    ("INTC", "Intel Corporation", 139.78, 8.93, "HOLD", "芯片", "YTD+240% | 18A工艺上线 | Apple代工传闻", "18A里程碑验证+Apple潜在代工订单，估值修复空间仍大"),
    ("GOOGL", "Alphabet Inc.", 357.08, 5.84, "HOLD", "应用", "Gemini 3.1 Ultra | 云收入增速26% | 搜索AI集成", "Gemini生态+TPU自研+搜索AI化，三层护城河 intact"),
    ("MSFT", "Microsoft Corp.", 372.83, -0.04, "HOLD", "应用", "Azure增速31% | Copilot ARR>$10B | OpenAI深度绑定", "企业AI消费最高确定性，Copilot生态粘性构建中"),
    ("META", "Meta Platforms", 563.05, 2.33, "HOLD", "应用", "Llama 4开源 | Reels变现加速 | AI推荐引擎驱动DAU", "开源模型战略+社交广告AI优化，AI应用层最大变现平台"),
    ("AAPL", "Apple Inc.", 289.12, 1.88, "HOLD", "应用", "iOS 27开放第三方AI | 服务端AI资本开支$10B+/年", "端侧AI入口价值被低估，iOS开放AI模型选择生态变革"),
    ("PLTR", "Palantir Technologies", 116.68, 3.32, "BUY", "应用", "AIP平台增速>50% | 政府合同扩张 | 估值溢价明显", "企业AI平台化最激进，但估值需警惕，适合高风险偏好"),
    ("SNOW", "Snowflake Inc.", 254.50, 1.13, "BUY", "应用", "Cortex AI集成 | 收入增长22% | 竞争加剧", "数据平台AI化转型中，但Databrick等竞争压力上升"),
    ("BABA", "Alibaba Group", 95.98, 1.23, "HOLD", "应用", "Qwen3 MoE | 阿里云增速14% | 通义千问DAU 2500万", "中国AI云龙头但增长放缓，关注Qwen3商业化进展"),
    ("TSLA", "Tesla Inc.", 420.58, 10.76, "HOLD", "应用", "FSD V13延迟 | Optimus量产2026 | 能源业务增长", "机器人+AI叙事 intact，但短期业绩波动大，需事件催化"),
    ("CEG", "Constellation Energy", 248.37, -5.93, "SPEC BUY", "能源", "核电重启+AI数据中心供电 | 订单积压$30B+ | 监管绿灯", "AI算力电力需求爆发最直接受益者，核电复兴核心标的"),
    ("CCJ", "Cameco Corp.", 101.86, -1.56, "SPEC BUY", "能源", "铀价$85/lb | 供给缺口持续 | 核电复兴原料端", "铀供需结构性缺口，核电复兴上游最直接杠杆"),
    ("OKLO", "Oklo Inc.", 52.33, -0.82, "BUY", "能源", "小型模块化反应堆 | Sam Altman背书 | 早期阶段高风险", "先进核反应堆技术路线，Altman个人押注，高风险高回报"),
]
...
def build_stock_card(s, highlight=False):
    ticker, name, price, pct, rec, cat, metrics, reason = s
    hl = ' highlight-stock' if highlight else ''
    ...
    return f'''    <div class="stock-card{hl}">
      ...
      <div class="price-row">
        ...
        <span class="change {change_class(pct)}">{change_str(pct)}</span>
      </div>
      <div class="stock-metrics">核心指标: {metrics}</div>
      <div class="stock-reason">推荐: {reason}</div>
    </div>'''

# S2 judgment (≤57 chars)
s2_judgment = "芯片财报前资金抢跑，AMD/AMAT领涨；核电回调，Q2业绩验证窗口开启。"
...
# Build HTML
stock_cards = "\n".join(build_stock_card(s, highlight=(s[0]=="NVDA")) for s in stocks)

html = f'''<!DOCTYPE html>
<html lang="zh-CN"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Agentic Market Daily | 2026-07-01</title>
<style>
:root {{
  --bg: #0a0e1a;
  --card: #0d1f35;
...
<header>
  ...
  <div class="date-badge">2026-07-01 | Wednesday | Asia/Shanghai 08:07</div>
</header>
...
<div class="section">
  <div class="section-title"><span class="num">2</span> 专家共识跨板块综合研判</div>
  <div class="insight-box">
    <span class="label">当日核心判断</span>
    <div class="content">{s2_judgment}</div>
  </div>
</div>
...
<div class="section">
  ...
  <table class="data-table">
    ...
    <tbody>
      <tr><td><strong>Anthropic</strong></td><td>Claude Mythos (即将发布)</td><td>预计SWE-bench 85%+, 2M context</td><td>2026H2</td><td>代码能力行业最强，推理cost比GPT-5.5低40%</td><td>企业级Agent首选，估值$75B，IPO候选</td></tr>
      <tr><td><strong>OpenAI</strong></td><td>GPT-5.5 / Spud</td><td>Spud预训练完成，预计4-6周内发布</td><td>2026-06</td><td>统一模型策略：单模型覆盖全场景</td><td>$500B revenue by 2027, 微软依赖度降至25%</td></tr>
...
<div class="section">
  ...
  <table class="data-table">
    ...
    <tbody>
      <tr><td><strong>NVIDIA</strong></td><td>GTC 2026: $1T AI机会</td><td>$1T数据中心基建至2027, Blackwell热销</td><td><span class="change up">+4.06%</span></td><td>2026-03-17</td><td>NVIDIA GTC 2026</td><td>核心持仓BUY — 数据中心收入增速25%，Blackwell渗透率仅15%，Rubin 2026H2量产提供第二增长曲线</td></tr>
      <tr><td><strong>AMD</strong></td><td>MI400系列挑战NVDA</td><td>MI400 GPU+EPYC Venice, 数据中心收入$30B+</td><td><span class="change up">+11.59%</span></td><td>2026-06-04</td><td>Tech Insider</td><td>BUY — MI400系列在推理性价比上挑战NVDA，Lisa Su指引TAM年增35%</td></tr>
      <tr><td><strong>Intel</strong></td><td>Q1 2026收入$13.6B</td><td>$13.6B, +7% YoY, DCAI $5.1B +22%</td><td><span class="change up">+8.93%</span></td><td>2026-04-24</td><td>Intel Earnings</td><td>HOLD — 18A里程碑验证+Apple潜在代工订单，估值修复空间仍大</td></tr>
    </tbody>
  </table>
  ...
  <div class="insight-box">
    ...
    <div class="content">
      <strong>AMD追赶NVDA，Intel追赶台积电：</strong>AMD Q1收入增速38%（vs NVDA 25%），MI300系列在Meta/Lambda/Hexaforce出货，MI450将在2026H2对位NVDA B300。Intel 18A良率从60%提升至85%，距离量产只差2个百分点。对投资者的结构性启示：NVDA仍是"确定性溢价"标的（BUY核心持仓），AMD是"估值修复"标的（BUY），Intel是"深度价值反转"（HOLD）——三者并非互斥。
    </div>
  </div>
</div>
...
<div class="section">
  ...
  <table class="data-table">
    ...
    <tbody>
      <tr><td><strong>编程Agent</strong></td><td>Cursor $2B ARR, Cognition $1B融资</td><td>Cursor ARR $2B, Cognition估值$26B</td><td>TechCrunch/Entrepreneur Loop 2026-05</td><td>MSFT (GitHub Copilot) BUY — 编程Agent是最高ROI场景</td></tr>
      ...
      <tr><td><strong>Robotics Agent</strong></td><td>Tesla Optimus Gen 3, Figure AI</td><td>Figure 10k units 2026, Tesla H2</td><td>Tesla/Figure</td><td>TSLA HOLD — 硬件成本$20K/unit是瓶颈</td></tr>
    </tbody>
  </table>
  ...
  <div class="insight-box">
    ...
    <div class="content">
      <strong>编程Agent是AI应用最先达到PMF（Product-Market Fit）的场景：</strong>Cursor $2B ARR和Cognition $1B融资验证编程Agent商业化可行性。Claude Code和GitHub Copilot的付费转化率>30%，远高于其他Agent形态。第二梯队是电商Agent（淘宝AI导购）和科研Agent（Deep Research）。投资优先级：编程Agent（MSFT）> 电商Agent（BABA）> 科研Agent（GOOGL）> 搜索Agent（风险）。
    </div>
  </div>
</div>
...
<div class="section">
  ...
  <table class="data-table">
    ...
    <tbody>
      ...
      <tr><td><strong>A2A</strong></td><td>Google推动，Workspace集成</td><td>Gmail/Docs/Sheets Agent互通</td><td>2026-06-21</td><td>GOOGL BUY — A2A=Google Workspace护城河</td></tr>
...
<div class="section">
  ...
  <table class="data-table">
    ...
    <tbody>
      ...
      <tr><td><strong>AI PC</strong></td><td>Copilot+ PC, Intel Lunar Lake</td><td>Copilot+ 20M units shipped</td><td>NPU从10 TOPS提升至50+ TOPS</td><td>INTC HOLD — Lunar Lake=AI PC转折点</td></tr>
      <tr><td><strong>AI眼镜</strong></td><td>Meta Ray-Ban, Apple Glass (2026)</td><td>Meta glasses 3M units sold</td><td>轻量Agent=语音+视觉</td><td>META HOLD — 眼镜=下一代计算平台候选</td></tr>
...
<div class="section">
  ...
  <table class="data-table">
    ...
    <tbody>
      <tr><td><strong>半导体设备</strong></td><td>ASML High-NA EUV订单</td><td>EXE:5000 $500M/unit, backlog >50 units</td><td>ASML Investor Day 2026-04</td><td>ASML HOLD — 唯一High-NA供应商，垄断性定价权</td></tr>
      <tr><td><strong>HBM内存</strong><td>HBM3E→HBM4迁移</td><td>HBM3E 80% 2026 share, HBM4 2026H2 ramp</td><td>SK Hynix Q1 Earnings</td><td>MU HOLD — HBM4市场份额争夺决定存储龙头</td></tr>
...
<div class="section">
  ...
  <div class="insight-box">
    ...
    <div class="content">
      <strong>1. AMD vs NVDA：估值修复 vs 确定性溢价</strong> — AMD Q1收入+38% YoY（vs NVDA +25%），MI450将在2026H2对位B300。但NVDA Blackwell渗透率仅15%，Rubin提供第二曲线。评级：AMD=BUY（估值修复），NVDA=核心持仓BUY（确定性溢价）。<br><br>
      <strong>2. 微信Agent vs 豆包Agent：ToC分发战争</strong> — 腾讯WeChat内置Agent（QClaw）获得14亿MAU，字节豆包获得抖音内容闭环。2026H2关键变量：微信Agent能否突破"聊天机器人"形态？<br><br>
...
<div class="footer">
  <p>Agentic Market Daily | 半导体投资级技术情报 · 每日晨报</p>
  <p>2026-07-01 | 数据来源: ifind实时API, Yahoo Finance, SEC Filings, Company Earnings</p>
  <p>本报告仅供信息参考，不构成投资建议</p>
</div>
...
print(f"S2 judgment ({len(s2_judgment)} chars): {s2_judgment}")
