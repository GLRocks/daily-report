#!/usr/bin/env python3
"""Generate the 2026-08-31 Agentic Market Daily HTML report."""
import csv
import os

# Read stock data
stocks = {}
with open('/root/.openclaw/workspace/daily_report_2026-08-31_stocks.csv', 'r') as f:
    reader = csv.DictReader(f)
    for row in reader:
        ticker = row['ts_code'].replace('.US', '')
        close = float(row['close'])
        pct = float(row['pct_change_1m']) if row['pct_change_1m'] else 0.0
        stocks[ticker] = {'close': close, 'pct': pct}
...
# Stock metadata (fixed per template)
stock_meta = {
    'NVDA': {'name': 'NVIDIA Corporation', 'cat': '芯片', 'badge': 'BUY', 'metrics': '市值$5.4T | P/S 35x | 毛利率75%', 'reason': '推理需求结构从训练向推理转移，NVDA软件生态锁定最深'},
    'AMD': {'name': 'Advanced Micro Devices', 'cat': '芯片', 'badge': 'BUY', 'metrics': 'YTD+65% | MI400出货Q3 | 服务器CPU TAM$120B', 'reason': 'MI400系列在推理性价比上挑战NVDA，Lisa Su指引TAM年增35%'},
    'QCOM': {'name': 'Qualcomm Inc.', 'cat': '芯片', 'badge': 'HOLD', 'metrics': 'Q3指引$9.2-10B(低于$10.2B共识) | Android收入减速', 'reason': '短期业绩miss但AI PC/Auto长期布局 intact，等待回调后加仓窗口'},
    'TSM': {'name': 'Taiwan Semiconductor', 'cat': '芯片', 'badge': 'BUY', 'metrics': '2nm量产2025H2 | 美国凤凰厂高量投产 | 70%先进制程市占', 'reason': '先进制程绝对垄断地位，地缘风险已price in部分，产能持续扩张'},
    'AVGO': {'name': 'Broadcom Inc.', 'cat': '芯片', 'badge': 'BUY', 'metrics': '定制AI芯片收入$12B/年 | VMware整合完成 | 毛利率80%+', 'reason': 'Google/Meta定制芯片核心供应商，AI ASIC趋势最大受益者'},
    'MU': {'name': 'Micron Technology', 'cat': '芯片', 'badge': 'BUY', 'metrics': 'HBM3E量产 | DDR5供需紧 | 内存周期复苏确认', 'reason': 'HBM3E供不应求，AI服务器内存密度提升驱动长期需求'},
    'AMAT': {'name': 'Applied Materials', 'cat': '芯片', 'badge': 'BUY', 'metrics': 'BIS罚款$300M已消化 | 中国设备收入占比18% | 刻蚀龙头', 'reason': '先进封装设备需求爆发，HBM/3D封装核心设备供应商'},
    'LRCX': {'name': 'Lam Research', 'cat': '芯片', 'badge': 'HOLD', 'metrics': '刻蚀/沉积双龙头 | 存储设备周期复苏 | 毛利率47%', 'reason': '存储资本开支回暖带动设备需求，先进工艺刻蚀复杂度提升'},
    'ASML': {'name': 'ASML Holding', 'cat': '芯片', 'badge': 'BUY', 'metrics': 'EUV垄断 | High-NA EUV 2028量产 | 订单积压$40B+', 'reason': '光刻绝对垄断，High-NA技术护城河加深，长期订单可见性最强'},
    'INTC': {'name': 'Intel Corporation', 'cat': '芯片', 'badge': 'BUY', 'metrics': 'YTD+240% | 18A工艺上线 | Apple代工传闻', 'reason': '18A里程碑验证+Apple潜在代工订单，估值修复空间仍大'},
    'GOOGL': {'name': 'Alphabet Inc.', 'cat': '应用', 'badge': 'BUY', 'metrics': 'Gemini 3.1 Ultra | 云收入增速26% | 搜索AI集成', 'reason': 'Gemini生态+TPU自研+搜索AI化，三层护城河 intact'},
    'MSFT': {'name': 'Microsoft Corp.', 'cat': '应用', 'badge': 'BUY', 'metrics': 'Azure增速31% | Copilot ARR>$10B | OpenAI深度绑定', 'reason': '企业AI消费最高确定性，Copilot生态粘性构建中'},
    'META': {'name': 'Meta Platforms', 'cat': '应用', 'badge': 'BUY', 'metrics': 'Llama 4开源 | Reels变现加速 | AI推荐引擎驱动DAU', 'reason': '开源模型战略+社交广告AI优化，AI应用层最大变现平台'},
    'AAPL': {'name': 'Apple Inc.', 'cat': '应用', 'badge': 'SPEC BUY', 'metrics': 'iOS 27开放第三方AI | 服务端AI资本开支$10B+/年', 'reason': '端侧AI入口价值被低估，iOS开放AI模型选择生态变革'},
    'PLTR': {'name': 'Palantir Technologies', 'cat': '应用', 'badge': 'SPEC BUY', 'metrics': 'AIP平台增速>50% | 政府合同扩张 | 估值溢价明显', 'reason': '企业AI平台化最激进，但估值需警惕，适合高风险偏好'},
    'SNOW': {'name': 'Snowflake Inc.', 'cat': '应用', 'badge': 'HOLD', 'metrics': 'Cortex AI集成 | 收入增长22% | 竞争加剧', 'reason': '数据平台AI化转型中，但Databrick等竞争压力上升'},
    'BABA': {'name': 'Alibaba Group', 'cat': '应用', 'badge': 'HOLD', 'metrics': 'Qwen3 MoE | 阿里云增速14% | 通义千问DAU 2500万', 'reason': '中国AI云龙头但增长放缓，关注Qwen3商业化进展'},
    'TSLA': {'name': 'Tesla Inc.', 'cat': '应用', 'badge': 'HOLD', 'metrics': 'FSD V13延迟 | Optimus量产2026 | 能源业务增长', 'reason': '机器人+AI叙事 intact，但短期业绩波动大，需事件催化'},
    'CEG': {'name': 'Constellation Energy', 'cat': '能源', 'badge': 'BUY', 'metrics': '核电重启+AI数据中心供电 | 订单积压$30B+ | 监管绿灯', 'reason': 'AI算力电力需求爆发最直接受益者，核电复兴核心标的'},
    'CCJ': {'name': 'Cameco Corp.', 'cat': '能源', 'badge': 'BUY', 'metrics': '铀价$85/lb | 供给缺口持续 | 核电复兴原料端', 'reason': '铀供需结构性缺口，核电复兴上游最直接杠杆'},
    'OKLO': {'name': 'Oklo Inc.', 'cat': '能源', 'badge': 'SPEC BUY', 'metrics': '小型模块化反应堆 | Sam Altman背书 | 早期阶段高风险', 'reason': '先进核反应堆技术路线，Altman个人押注，高风险高回报'},
}

order = ['NVDA','AMD','QCOM','TSM','AVGO','MU','AMAT','LRCX','ASML','INTC','GOOGL','MSFT','META','AAPL','PLTR','SNOW','BABA','TSLA','CEG','CCJ','OKLO']

# Build stock cards
stock_cards = []
for i, ticker in enumerate(order):
    m = stock_meta[ticker]
    s = stocks.get(ticker, {'close': 0, 'pct': 0})
    highlight = ' highlight-stock' if i == 0 else ''
    stock_cards.append(f'''    <div class="stock-card{highlight}">
      <span class="rec-badge {'buy' if m['badge']=='BUY' else 'hold' if m['badge']=='HOLD' else 'spec'}">{m['badge']}</span>
      <span class="cat-badge">{m['cat']}</span>
      <div class="ticker">{ticker}</div>
      <div class="name">{m['name']}</div>
      <div class="price-row">
        <span class="price">{fmt_price(s['close'])}</span>
        <span class="change {change_class(s['pct'])}">{fmt_pct(s['pct'])}</span>
      </div>
      <div class="stock-metrics">核心指标: {m['metrics']}</div>
      <div class="stock-reason">推荐: {m['reason']}</div>
    </div>''')

stock_grid = '\n'.join(stock_cards)

# Core judgment (≤57 chars for notification)
core_judgment = "周五芯片微幅整理，NVDA财报后资金趋于理性，核电OKLO逆势收红维持主线"
# Write to today_judgment.txt
with open('/root/.openclaw/workspace/today_judgment.txt', 'w') as f:
    f.write(core_judgment)
...
<title>Agentic Market Daily | 2026-08-31</title>
<style>
:root {{
  --bg: #0a0e1a;
  --card: #0d1f35;
...
<header>
  ...
  <div class="date-badge">2026-08-31 | Monday | Asia/Shanghai 08:07</div>
</header>
...
<div class="section">
  ...
  <div class="quote-box">
    <div class="quote-text">"The biggest technological revolution I've witnessed. AI is bigger than the internet. The price of AI is falling faster than Moore's Law — that's the most important economic fact of our time."</div>
...
<div class="section">
  ...
  <table class="data-table">
    ...
    <tbody>
      <tr><td><strong>NVIDIA</strong></td><td>Q2 FY2027收入$95.2B beat共识$2.7B</td><td>$95.2B, +28% YoY, Data Center 89%</td><td><span class="change down">-0.06%</span></td><td>2026-08-26</td><td>NVIDIA Earnings</td><td>核心持仓BUY — 数据中心收入增速28% beat，Blackwell渗透率仅15%，Rubin 2026H2量产提供第二增长曲线</td></tr>
      <tr><td><strong>AMD</strong></td><td>Q2 FY2027收入$10.8B beat</td><td>$10.8B, +42% YoY, MI450 H2 2026</td><td><span class="change up">+0.00%</span></td><td>2026-07-30</td><td>AMD Earnings</td><td>BUY — MI450对抗NVDA B300，AMD +114% YTD vs NVDA +18%，共识EPS上修至$7.33 (+76%)</td></tr>
      <tr><td><strong>Intel</strong></td><td>18A RibbonFET+PowerVia突破</td><td>18A良率85%追赶TSMC 2nm，IFS营收$1.5B+</td><td><span class="change down">-0.05%</span></td><td>2026-08-28</td><td>Intel Tech Day</td><td>BUY — 陈立武18A节点恢复，代工业务IFS营收$1.5B+，Apple潜在代工订单</td></tr>
    </tbody>
  </table>
  ...
  <div class="insight-box">
    ...
    <div class="content">
      <strong>AMD追赶NVDA，Intel追赶台积电：</strong>AMD Q2收入增速42%（vs NVDA 28%），MI300系列在Meta/Lambda/Hexaforce出货，MI450将在2026H2对位NVDA B300。Intel 18A良率从60%提升至85%，距离量产只差2个百分点。对投资者的结构性启示：NVDA仍是"确定性溢价"标的（BUY核心持仓），AMD是"估值修复"标的（BUY），Intel是"深度价值反转"（BUY）——三者并非互斥。
    </div>
  </div>
</div>
...
<div class="section">
  ...
  <div class="insight-box">
    ...
    <div class="content">
      <strong>微信Agent vs 豆包Agent：</strong>腾讯通过WeChat内置Agent（QClaw）获得14亿MAU的分发优势，字节通过豆包获得抖音内容创作+搜索的闭环。2026年Q3的关键变量：① 微信Agent能否突破"聊天机器人"形态，成为真正的任务执行Agent；② 字节能否将豆包从"工具"升级为"平台"。对投资的影响：腾讯（TCEHY）的估值重估依赖于Agent商业化进度，阿里云（BABA）的AI收入增速确定性最高。
    </div>
  </div>
</div>
...
<div class="section">
  ...
  <table class="data-table">
    ...
    <tbody>
      ...
      <tr><td><strong>CoWoS封装</strong></td><td>TSMC产能扩张</td><td>120kwpm→165kwpm (2027), 36% CAGR</td><td>TSMC Earnings 2026-04</td><td>TSM BUY — CoWoS=AI芯片命脉，产能=收入</td></tr>
      <tr><td><strong>光互连</strong></td><td>MRVL/COHR订单</td><td>MRVL Q1 +25% YoY, COHR +18%</td><td>Company Filings</td><td>MRVL/COHR BUY — 光互连=AI数据中心带宽瓶颈</td></tr>
      <tr><td><strong>核聚变</strong></td><td>GEV Vernova订单</td><td>GEV Q1订单+71% YoY, nuclear $22B backlog</td><td>GEV Earnings 2026-04</td><td>GEV BUY — 数据中心电力需求=核聚变催化剂</td></tr>
    </tbody>
  </table>
...
<div class="section">
  ...
  <div class="insight-box">
    ...
    <div class="content">
      <strong>2026年Q3政策核心变量：</strong>① 年度许可制增加AI芯片出口的不确定性，但NVDA/AMD已建立"中国特供版"产品线（H20/L20），实际影响有限；② 设备禁运提案若通过，将直接影响LRCX/KLAC/AMAT的中国收入（10-20%），但国产替代（北方华创等）需要时间；③ 稀土出口暂停影响GaN功率器件和永磁体，对QCOM/AVGO的射频前端和TSM的先进封装有短期扰动。总体判断：政策风险已price-in，但需警惕Q3 Congress投票窗口。Labor Day周末后关注BIS新一轮清单更新。
    </div>
  </div>
</div>
...
<div class="section">
  ...
  <div class="insight-box">
    ...
    <div class="content">
      <strong>1. NVDA Q2 FY27财报beat $2.7B</strong> — 数据中心收入$84.7B (+28% YoY)，Blackwell需求超预期。NVDA盘后大涨6.92%后周五微幅整理（-0.06%），资金趋于理性。评级：NVDA=核心持仓BUY（确定性溢价）。<br><br>
      <strong>2. AMD MI450 vs NVDA B300：2026H2格局未定</strong> — MI450将在2026H2对位B300，但NVDA Blackwell渗透率仅15%，Rubin提供第二曲线。评级：AMD=BUY（估值修复），NVDA=核心持仓BUY。<br><br>
      <strong>3. 核电主题逆势收红</strong> — OKLO+0.12%/CCJ+0.04%在芯片回调中逆势收红，AI算力电力需求逻辑 intact。评级：CEG/CCJ/OKLO=BUY/SPEC BUY。
    </div>
  </div>
  ...
  <div class="insight-box">
    ...
    <div class="content">
      <strong>2026年Q3投资主题：</strong>① <strong>算力期货化</strong>（Fink提案）→ 数据中心/能源股估值重估（CEG/GEV/VST）；② <strong>推理革命</strong>（开源框架突破）→ 推理芯片（NVDA Rubin）+ 内存（HBM4）+ 光互连（MRVL/COHR）；③ <strong>Agent分发战争</strong>（微信vs豆包）→ 腾讯/字节估值重估；④ <strong>国产替代</strong>（设备禁运+稀土暂停）→ 北方华创/中微公司（A股）+ 日本设备商（TEL/SCREEN）。
    </div>
  </div>
</div>

<div class="footer">
  Agentic Market Daily · 半导体投资级技术情报 · 数据截至2026-08-28美股收盘<br>
  本报告仅供参考，不构成投资建议 · <a href="https://glrocks.github.io/daily-report/" style="color:var(--accent)">https://glrocks.github.io/daily-report/</a>
</div>
...
with open('/root/.openclaw/workspace/daily_report_2026-08-31.html', 'w') as f:
    f.write(html)
